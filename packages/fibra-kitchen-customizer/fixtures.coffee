  ...
  'PR':
    ...
    'sections': [
      ...
      }
      {
        ...
        short_title: 'Sponsor- & Partnership'
      }
      {
        'cat': '10'
        'nr': '4'
    ...
    'sections': [
      {
        ...
        short_title: 'Use of Traditional Media for DM'
      }
      {
        'cat': '13'
        'nr': '2'
        'title': 'Use of New Media for Direct Marketing'
        short_title: 'Use of New Media for DM'
      }
    ]
  'EVENTS':
    'nr': '14'
...
complete_categories_no_sections= ->
  for cat, def of cats
    unless def.sections.length >0
      def.sections.push
        cat: def.nr
        nr: '1'
        title: cat
...
make_sections= ->
  ...
  color_count = 0
  for cat, def of cats
    color_count+=def.sections.length
  console.log "Got #{color_count} sections"
  next=0
  spacing = colors.length/color_count
  ret= []
  colors.forEach (color, idx)->
    if idx==next
      ret.push color
      next = Math.round spacing*ret.length
  console.log('picked #{ret.length} colors')
  idx=0
  for cat, def of cats
    def.sections =def.sections.map (sec_def)->
      sec_def.color = ret[idx]
      console.log "adding color #{ret[idx]}"
      idx++
      return sec_def
  return ret
make_sections()
    ...
    unless def?.sections?.length
      ...
      for sec_def in def.sections
          ...
          sec=
            ...
            category_id: cat._id
            color: sec_def.color
            short_name: sec_def.short_title or sec_def.title
        if def.sections.length >1 
          sec.name = "#{def.nr}.#{sec_def.nr} #{cat_name} - #{sec_def.title}"
        else
          sec.name= "#{def.nr} #{cat_name}"
